...
void ContactsKopete::activate(int what)
{
    if (d->interface && !d->noOnlineContacts) {
        QString contact = data(index(what, 0)).toString();
        d->interface->openChat(contact);
    } else {
        BaseModel::activate(what);
    }
    ...
    if (!d->interface) {
        ...
        if (forceReload) {
            ...
            if (count() == 0) {
                ...
                setSelfTitle(i18nc("Contacts (number of online contacts)", "Contacts (%1)", QString::number(count())));
            }

        }
    }

    // TODO: setEmitInhibited(false);
    // TODO: emit updated();
}
