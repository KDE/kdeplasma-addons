...
Devices::Type Devices::filter() const
{
    return d->filter;
}

void Devices::setFilter(Devices::Type type)
{
    clear();

    kDebug() << "Setting filter to" << type;
    d->filter = type;

    switch (type) {
        case Devices::Removable:
