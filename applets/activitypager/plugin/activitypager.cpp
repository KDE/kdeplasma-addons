...
ActivityPager::ActivityPager(QObject *parent)
    : QObject(parent),
...
void ActivityPager::setCurrentDesktop(int desktop)
{
...
void ActivityPager::setShowWindowIcons(bool show)
{
...
Qt::Orientation ActivityPager::orientation() const
{
    return m_orientation;
}

void ActivityPager::setOrientation(Qt::Orientation orientation)
{
...
QSizeF ActivityPager::size() const
{
    return m_size;
}

QSize ActivityPager::preferredSize() const
{
    return m_preferredSize;
}

void ActivityPager::setSize(const QSizeF &size)
{
    if (m_size == size) {
        return;
    }
...
ActivityPager::CurrentDesktopSelected ActivityPager::currentDesktopSelected() const
{
    return m_currentDesktopSelected;
}

void ActivityPager::setCurrentDesktopSelected(CurrentDesktopSelected cur)
{
...
ActivityPager::DisplayedText ActivityPager::displayedText() const
{
    return m_displayedText;
}

void ActivityPager::setDisplayedText(ActivityPager::DisplayedText disp)
{
...
#if HAVE_X11
void ActivityPager::slotAddDesktop()
{
    if (!m_isX11) {
        return;
    }
...
void ActivityPager::slotRemoveDesktop()
{
    if (!m_isX11) {
        return;
    }
...
void ActivityPager::recalculateGridSizes(int rows)
{
...
void ActivityPager::updateSizes()
{
...
void ActivityPager::recalculateWindowRects()
{
    NETRootInfo info(QX11Info::connection(), NET::NumberOfDesktops | NET::DesktopNames, NET::WM2DesktopLayout);
    m_rows = info.desktopLayoutColumnsRows().height();
...
void ActivityPager::numberOfDesktopsChanged()
{
...
void ActivityPager::desktopNamesChanged()
{
    m_pagerModel->clearDesktopRects();
    m_validSizes = false;
    startTimer();
...
void ActivityPager::windowChanged(WId id, const unsigned long* dirty)
{
    Q_UNUSED(id)
...
void ActivityPager::desktopsSizeChanged()
{
    m_pagerModel->clearDesktopRects();
    m_validSizes = false;
    startTimer();
...
void ActivityPager::startTimerFast()
{
    if (!m_timer->isActive()) {
...
void ActivityPager::moveWindow(int window, double x, double y, int targetDesktop, int sourceDesktop)
{
#if HAVE_X11
    if (!m_isX11) {
        return;
    ...
    // use _NET_MOVERESIZE_WINDOW rather than plain move, so that the WM knows this is a ActivityPager request
...
void ActivityPager::changeDesktop(int newDesktop)
{
#if HAVE_X11
    if (!m_isX11) {
        return;
...
QRect ActivityPager::fixViewportPosition( const QRect& r )
{
